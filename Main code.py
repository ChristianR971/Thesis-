# -*- coding: utf-8 -*-
"""
Created on Wed Jan 22 16:31:43 2025
...
## First copy/paste the new_GHG_account Excel file in the Aggregations folder
path_extensions = 'Aggregations/new_GHG_account.xlsx'
units = pd.read_excel(path_extensions, sheet_name='units',index_col=[0],header=[0])
data_exio.add_extensions(
    io=path_extensions,
    units=units,
    matrix='E')
...
#%%
# Test
#data_exio # Check if Satellite account has changed from 1104 to 1105
data_exio.query(
    matrices='E',
    scenarios='baseline',
    ).loc['GHG emissions (GWP100)'].sum()
...
# Create horizontal bar plot with conditional coloring
y_pos = np.arange(len(delta_Emp_totals))
colors = ['blue' if x >= 0 else 'red' for x in delta_Emp_totals]
plt.barh(y_pos, delta_Emp_totals, color=colors, alpha=0.7)

# Calculate padding for text positioning
max_abs_value = max(abs(min(delta_Emp_totals)), abs(max(delta_Emp_totals)))
...
# Customize plot
plt.title("Employment shifts relative to the baseline", 
          pad=20, fontsize=18, fontweight='bold')
plt.xlabel("Employment (in thousands)", fontsize=14)
plt.ylabel("Interventions", fontsize=14)

# Set y-axis ticks with intervention names
plt.yticks(y_pos, intervention_names, fontsize=14)
...
plt.xlim(x_min * 1.05, x_max * 1.05)  # Add 10% padding on both sides
...
colors = ['red' if x >= 0 else 'green' for x in delta_E_millions]
bars = ax.barh(y_pos, delta_E_millions, color=colors, alpha=0.7)
...
# Customize plot
ax.set_title("GHG emissions shift relative to the baseline", 
             pad=20, fontsize=18, fontweight='bold')
ax.set_xlabel("CO₂eq (million kg)", fontsize=16)
ax.set_ylabel("Interventions", fontsize=16)

# Set y-axis ticks with intervention names
ax.set_yticks(y_pos)
ax.set_yticklabels(intervention_names, fontsize=14)
...
colors = ['gold' if x >= 0 else 'red' for x in delta_VA_totals]
bars = ax.barh(y_pos, delta_VA_totals, color=colors, alpha=0.7)
...
# Customize plot
ax.set_title("Value Added shift relative to the Baseline", 
             pad=20, fontsize=18, fontweight='bold')
ax.set_xlabel("Value Added (Million €)", fontsize=16)
ax.set_ylabel("Interventions", fontsize=16)
...
data = {
    "Delta Emissions (tonnes)": [-133445246.7, -55670368.69, -443765389.2, -322227406.4, -11929365.34,
                -294962016.6, -1211036.994, -126079.5192, -611000.7581, -9800299.349,
                -37856338.05, 22871104.13, -318539489.7, -35632533.43, -334958556.1],
    "Delta Employment (1000p)": [9.114687303, -1.795646081, -8.75869875, -8.59932818, -0.384781324,
                  -7.871692884, -0.02785362, -0.001079969, -0.005233698, -0.225405011,
                  -0.389532033, 1.184360868, 3.36641353, 2.890169701, -3.41173282],
    "Delta Value-Added (M€)": [441.8022859, -69.66453618, -386.48829, -377.1394629, -14.92811568,
...
for row in range(df.shape[0]):
    for col in range(df.shape[1]):  # Iterate over all columns in df
        value = df.iloc[row, col]
        color = cmaps[col](norm_list[col](value))
        cell = table[(row + 1, col)]  # Update to align with table indices
...
data = {
    ...
    "Delta Emissions (tonnes)": [-133445246.7, -55670368.69, -443765389.2, -322227406.4, -11929365.34,-294962016.6, -1211036.994, -126079.5192, -611000.7581, -9800299.349,-37856338.05, 22871104.13, -318539489.7, -35632533.43, -334958556.1],
